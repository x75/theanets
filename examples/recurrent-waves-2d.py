...
cfs = [(i+1, 0.5 + i/10.) for i in range(DIM)]
SIN = [c * np.sin(TWOPI * f * T) for c, f in cfs] # , (4, 1.1))]
...
ZERO = np.zeros((BATCH_SIZE, len(T), DIM), 'f')
# WAVES_SIN = np.concatenate([SIN[None, :, None]] * BATCH_SIZE, axis=0).astype('f')
WAVES_SIN = np.concatenate([SIN.T[None,:,:]] * BATCH_SIZE, axis=0).astype('f')
...
ds = wavdataset(
    ...
    n_samples = 1,
    filename = "../../smp/playground/sequence/data/drinksonus44.wav")
# print("len(ds)", len(ds))
...
for i, layer in enumerate(networks):
    ...
    if args.mode == "mse":
        ...
        net = theanets.recurrent.Regressor([DIM, layer, DIM], loss="mse")
    elif args.mode == "mae":
        print("using MAE mode")
        net = theanets.recurrent.Regressor([DIM, layer, DIM], loss="mae")
    elif args.mode == "mdn":
    ...
    if os.path.exists(args.weights):
        ...
        for tm, _ in net.itertrain([INPUT, WAVES],
                               ...
                               weight_l2 = 0.5,
                               hidden_l1 = 0.5): #, save_progress="recurrent_waves_{}", save_every=100):
    ...
    inp = np.concatenate((INPUT[0], np.zeros((freerun_steps, DIM), dtype=np.float32))) # prd[0,-1,:].reshape((1,1,1))
